...
var updateResultActions = function() {
  results.forEach(function( person ) {
    ...
    } else if( event.rostered && (!person["w_" + event.rostered] )) {
 ...
 *     i -- indicates the event larp
 *     k -- indicates the event dizzy dizzy duck
 *     u -- indicates the event dingle dangle donut
 *     w -- indicates the event tug o war  
 *     j -- indicates the event egg joust
 *     m -- indicates the event bed race
 */
var eventOptions = [
  "Dodgeball:dct",
  "Volleyball:vct",
...
eventOptions = eventOptions.map(function( event, index ) {
  var result = /([A-Za-z0-9 ]+):(v|d|f|r|b|z|i|k|u|w|j|m)?(g|l|n)?(c?)(t?)/.exec( event ),
      w = result[3];
      e = result[2];
  var waivers = {
    "g": "general",
  ...
  return {
    ...
    tally:        !!result[4],
    tournament:   !!result[5],
    rostered:  tourna_olymp[e] //Which event roster a person must be on.
  };
});
